document.addEventListener("DOMContentLoaded", function() { //Ejecutamos desde el principio la llamada a la api
    ...
    fetch("https://pokeapi.co/api/v2/pokemon?limit=8&offset=143") //Llamada a la api de la primera generacion
    ...
    .then(data => {
        ...
        .then(pokemons => { //Pokemons es la resolucion de la primera promesa donde recuperamos los pokemons
            ...
            let text = "";
            pokemons.forEach((pokemon, index) => { //Recorro todos los pokemons hasta el 151
                if(pokemon.name !== "dratini" && pokemon.name !== "dragonair" && pokemon.name !== "dragonite"){// Compruebo que no son estos Pokémon debido a que solamente quiero introducir los Pokémon legendarios, por lo que se saltan.
                let fila = document.createElement('tr');//Creación de tr
                let columna1 = document.createElement('td');//Creación de td
                columna1.innerHTML =index+143;// Comienza la enumeración por el 143 hasta 150
                fila.appendChild(columna1);// Unión de columna a la fila
                let columna2 = document.createElement('td');//Creación de td
                columna2.innerHTML = pokemon.name.toUpperCase();// Introducción de nombre del Pokémon en Mayúsculas
                ...
                pokemon.types.forEach((type,index) => { //Recorro el array types de cada uno de los pokemons
                    ...
                    text += traductor(type.type.name);    // Se traduce el tipo de Pokémon
                    let columna3 = document.createElement('td');//Creación de td
                    columna3.innerHTML = text;// Se introduce el tipo de Pokémon
                    fila.appendChild(columna3);// Unión de columna a la fila
                    text = ""
